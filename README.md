...
```bash
ACP_SUBAGENT_CLAUDE_ENV_POLICY=allowlist
ACP_SUBAGENT_CLAUDE_ENV_ALLOWLIST='HOME,PATH,ANTHROPIC_*,CLAUDE_*,HTTPS_PROXY,HTTP_PROXY,NO_PROXY'
```
...
| `ACP_SUBAGENT_CLAUDE_ENV_POLICY` | Claude 子代理环境继承策略 | 继承全局策略 |
| `ACP_SUBAGENT_CLAUDE_ENV_ALLOWLIST` | Claude 子代理环境白名单 | 继承全局白名单 |
